...
bool Resource::Texture::Init(ID3D12Device* pDevice, D3D::DescriptorPool* pPool, const wchar_t* filename, DirectX::ResourceUploadBatch& batch)
{
    return Init(pDevice, pPool, filename, false, batch);
}

bool Resource::Texture::Init(ID3D12Device* pDevice, D3D::DescriptorPool* pPool, const wchar_t* filename, bool isSRGB, DirectX::ResourceUploadBatch& batch)
{
    // 引数チェック
    ...
    auto hr = DirectX::CreateDDSTextureFromFile(pDevice,
                                                batch,
                                                filename,
                                                m_pTex.GetAddressOf(),
                                                true,
...
bool Resource::Texture::Init(ID3D12Device* pDevice, D3D::DescriptorPool* pPool, const D3D12_RESOURCE_DESC* pDesc, D3D12_RESOURCE_STATES initState, bool isCube)
{
    return Init(pDevice, pPool, pDesc, initState, isCube, false);
}

bool Resource::Texture::Init(ID3D12Device* pDevice, D3D::DescriptorPool* pPool, const D3D12_RESOURCE_DESC* pDesc, D3D12_RESOURCE_STATES initState, bool isCube, bool isSRGB)
{
...
D3D12_CPU_DESCRIPTOR_HANDLE Resource::Texture::GetHandleCPU() const
{
    if (m_pHandle != nullptr)
    {
...
D3D12_GPU_DESCRIPTOR_HANDLE Resource::Texture::GetHandleGPU() const
{
    if (m_pHandle != nullptr)
    {
...
D3D12_SHADER_RESOURCE_VIEW_DESC Resource::Texture::GetViewDesc(bool isCube)
{
    ...
    case D3D12_RESOURCE_DIMENSION_TEXTURE3D:
        {
            ...
            viewDesc.Texture3D.MostDetailedMip     = desc.MipLevels;
